...
| 文字排版设置：字号、行距、上下/水平边距、段距、首行缩进、主题 | `4db9134d` 及其最终状态 | `ReaderSettingsView.swift`、`TextReaderPreferences.swift`、`TextPaginator.swift`、`ReaderTextView.swift`、分页/滚动文字控制器、设置默认值和本地化 | 不恢复 Readium 阅读器；把仍适用于上游原生 EPUB/TXT 文字阅读器的设置接入当前分页和滚动渲染链。本轮恢复双页只在书页外侧加入设备安全区，暂不拆分左右边距。 | `TextPaginatorTests` 增加设置相关检查；双页居中、旋转和状态区仍需设备验收。 |
...
| 文字可长按选择，朗读时翻页/滚动/滑块导航锁定 | `d2b9a519`、`478b2cb3` | `MarkdownView.swift`、文字页控制器、`ReaderViewController.swift` | 不新增应用层双击选词手势；恢复分页 `UITextView` 的系统选择，外层单击 tap zone 不取消正文触摸，并只与可选择文字视图的单击识别器同时成功。朗读时只锁定页面容器导航和工具栏滑块。 | 真机发现翻到新页后第一次点击被文字交互吞掉；已补聚焦手势策略测试，仍需复验第一次点按和长按选择。 |
| 分页文字常驻章节、时间、电量和页码，双页使用外侧安全区 | `00b49944` 的最终状态 | `ReaderPagedTextViewController.swift`、单双页文字控制器 | 该体验原本与朗读代码位于同一旧提交，第一次迁移只提取朗读链路而遗漏。本轮只恢复分页模式；滚动文字不常驻显示。 | 已完成 iPhone/iPad 验收；左右独立边距作为后续体验优化，不阻塞里程碑。 |
| 阅读器使用通用设置图标 | `9189e359` | `ReaderViewController.swift` | 保留当前上游共享阅读器外壳和按类型区分的设置内容，只把 `textformat.size` 恢复为通用 `slider.horizontal.3`。 | 已完成真机界面检查。 |
...
- 用户已在 iPhone 17（iOS 26）和 iPad Air 4 完成第一里程碑集中验收并接受当前迁移版本；首次点击补丁和新的排版体验项另见后续任务卡。
