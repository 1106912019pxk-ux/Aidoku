...
| 文字可长按选择，朗读时翻页/滚动/滑块导航锁定 | `d2b9a519`、`478b2cb3` | `MarkdownView.swift`、文字页控制器、`ReaderViewController.swift` | 不新增应用层双击选词手势；恢复分页 `UITextView` 的系统选择，并避免外层显隐栏手势取消正文触摸。朗读时只锁定页面容器导航和工具栏滑块。 | 真机反馈确认分页选择缺失；本轮已恢复分页交互，待验证长按选择不会触发翻页。 |
| 分页文字常驻章节、时间、电量和页码，双页使用外侧安全区 | `00b49944` 的最终状态 | `ReaderPagedTextViewController.swift`、单双页文字控制器 | 该体验原本与朗读代码位于同一旧提交，第一次迁移只提取朗读链路而遗漏。本轮只恢复分页模式；滚动文字不常驻显示。 | 已恢复代码，待 iPhone/iPad 显隐栏、旋转和双页视觉验收。 |
| 阅读器使用通用设置图标 | `9189e359` | `ReaderViewController.swift` | 保留当前上游共享阅读器外壳和按类型区分的设置内容，只把 `textformat.size` 恢复为通用 `slider.horizontal.3`。 | 已恢复代码，待界面检查。 |
