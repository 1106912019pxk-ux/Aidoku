...
- [x] 分页文字连续翻过至少 10 个新页面，每页第一次单击左/右 tap zone 都立即且只执行一次对应翻页。
- [x] 分页文字不响应长按或双击选文，双击不会出现文字选区或系统选择菜单。
- [x] 滚动文字原有选择行为不变；漫画原有单击/双击翻页行为不变。
- [x] 用户完成当前候选的云端编译与真机复验，并确认本轮验收通过。
...
- 2026-08-31：用户确认候选 `bc7fdbfe` 本轮真机验收通过，同意作为新的稳定版本合并到 `main`。

## 验证状态

- 已有真机证据：TXT 分页字体、左右独立边距和分页即时点击通过；分页文字不提供选择，漫画原有双击缩放保持正常；漫画进度暂未发现问题，仍需继续观察。
...
- 用户已确认当前候选本轮真机验收通过；本地 EPUB 旧封面迁移仍是明确非目标，漫画进度继续作为下一阶段观察项。
